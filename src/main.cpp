...
int nStakeMinConfirmations = nCoinbaseMaturity;
...
const string strMessageMagic = "DraculaCoin Signed Message:\n";
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (!txdb.ReadCheckpointPubKey(strPubKey) || strPubKey != std::string(Params().GetCheckpointPubKey().begin(),Params().GetCheckpointPubKey().end()))
    {
        // write checkpoint master key to db
        txdb.TxnBegin();
        if (!txdb.WriteCheckpointPubKey(std::string(Params().GetCheckpointPubKey().begin(),Params().GetCheckpointPubKey().end())))
